...
在 Cyber Tank 的移动能力基础上扩展室内移动能力，加入障碍识别和路径规划。

**核心路线：依赖拓竹生态 + OpenClaw，打造私人定制智能家居硬件拓展 kit。**

### 阶段一：IoT 控制现有设备（可立刻落地）

0-1 通过 WiFi/MQTT 接入现有智能家居生态（米家、涂鸦、HomeAssistant 等），实现对灯、门锁、空调等设备的语音和自动化控制。OpenClaw 作为大脑统一调度，Cyber Bricks 作为执行层。这一步让 0-1 从「一个固定位置的机器人」变成「一个能在家里移动的陪伴者」，同时通过远程控制门锁等设备，在主人离家/到家时自动开关门。

### 阶段二：私人定制硬件拓展 kit（ Maker 方式打造）

随着需求明确，用拓竹生态（Bambu Suite 激光/刀切 + H2C 3D 打印 + Cyber Bricks 运动控制）自己设计并制造专属的硬件 kit。例如：
- 专属的门锁控制模组（替代购买第三方网关）
- 针对特定电器的红外/射频控制模组
- 定制传感器阵列（用于特定场景感知）
