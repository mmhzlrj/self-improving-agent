...
详见：`memory/2026-03-31-Q-AResearch.md`

### Q1: AS5600 - 作用 + Jetson Nano 能否替代
- **作用**：12位磁性编码器，实时反馈关节角度，MT3轨迹录制必需
- **Jetson Nano**：技术上可接AS5600，但需改ESP32固件，不如AS5600直接方案
- **结论**：AS5600是根本解，B级优先

### Q2: Cyber Bricks 关节反馈（4个方案）
| 方案 | 成本 | 速度 | 精度 |
|------|------|------|------|
| A 电流检测 | ~$1 | 最快 | 堵转检测 |
| B 视觉ML | $0 | 中期 | ±5° |
...
### Q3: pointnet2_ops 安装
- **重大发现**：部署不需要pointnet2_ops，只在训练用
- **已实测**：MT3推理模块全部可加载，RTX 2060可跑
- **结论**：不需要安装 ✅

### Q4: iPhone 室内模型
- **iPhone 16 Pro**：LiDAR ±5mm，RoomPlan生成.usdz，60fps深度
- **B序列已完成**：数据通道已通（延迟测试通过）
- **待做**：Q4-1 RoomPlan集成导出，B/C级

---

## 下一步行动

1. **短期（今天）**: 确认 A+D 关节反馈方案（不改硬件快速验证）
2. **中期（本周）**: Q4-1 RoomPlan集成导出（iPhone已有能力）
3. **长期（本月）**: AS5600采购 + CyberBricks固件改造
