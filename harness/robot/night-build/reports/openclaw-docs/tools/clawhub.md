...
`CLAWHUB_WORKDIR`). OpenClaw loads workspace skills from `/skills`
...
* `--workdir `: Working directory (default: current dir; falls back to OpenClaw workspace).
* `--dir `: Skills directory, relative to workdir (default: `skills`).
* `--site `: Site base URL (browser login).
...
* `clawhub login` (browser flow) or `clawhub login --token `
* `clawhub logout`
* `clawhub whoami`
...
* `--label `: Label stored for browser login tokens (default: `CLI token`).
...
* `clawhub update `
* `clawhub update --all`
* `--version `: Update to a specific version (single slug only).
