...
* OpenClaw resolves fast mode in this order:
  ...
  4. Per-model config: `agents.defaults.models["/"].params.fastMode`
  5. Fallback: `off`
...
* When verbose is on, agents that emit structured tool results (Pi, other JSON agents) send each tool call back as its own metadata-only message, prefixed with ` : ` when available (path/command). These tool summaries are sent as soon as each tool starts (separate bubbles), not as streaming deltas.
...
* To change the session default, send a `/think:` directive (as before); the selector will reflect it after the next reload.
