...
`/skills` (highest) → `~/.openclaw/skills` → bundled skills (lowest)
...
OpenClaw picks that up as `/skills` on the next session.
...
* `primaryEnv` — env var name associated with `skills.entries..apiKey`.
...
* Download installs: `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`), `extract` (default: auto when archive detected), `stripComponents`, `targetDir` (default: `~/.openclaw/tools/`).
...
2. Applies any `skills.entries..env` or `skills.entries..apiKey` to
   `process.env`.
