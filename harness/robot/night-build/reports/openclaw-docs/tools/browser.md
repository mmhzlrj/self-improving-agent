...
* Set `browser.profiles..userDataDir` when an existing-session profile
...
* `x-openclaw-password: ` or HTTP Basic auth with that password
...
* `snapshot`:
  ...
  * `--frame ""` scopes role snapshots to an iframe (pairs with role refs like `e12`).
  * `--interactive` outputs a flat, easy-to-pick list of interactive elements (best for driving actions).
  * `--labels` adds a viewport-only screenshot with overlayed ref labels (prints `MEDIA:`).
...
2. Use `click ` / `type ` (prefer role refs in interactive mode)
3. If it still fails: `openclaw browser highlight ` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   * `openclaw browser trace start`
   * reproduce the issue
   * `openclaw browser trace stop` (prints `TRACE:`)
...
* Geolocation: `set geo   --origin "https://example.com"` (or `--clear`)
