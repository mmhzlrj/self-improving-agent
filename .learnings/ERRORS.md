...
- 改为 `profile: "full"` → 无 core entries → allow=void → 全部允许

### 教训

- alsoAllow 必须放在 `tools` 级别（global），不能放在 agent 级别
- `profile: "full"` = 无限制，`profile: "coding"` = 有限制
- "unknown entries" 警告不一定是错误，要看剥离后的效果
- 花 2 小时排查说明对工具策略 pipeline 不熟悉
...
### 根因
- 想走捷径省时间，反而浪费了更多时间
- 截断输出会掩盖重要信息（尾部内容、前期漏看等）

### 教训
- 调研阶段：先 `wc -l` 确认文件大小，再决定读取策略
- 大文件用 offset/limit 分段读取，但必须覆盖完整、不漏任何章节
