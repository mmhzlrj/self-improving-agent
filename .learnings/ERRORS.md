...
## 2026-05-18 12:52 — Kimi 浏览器自动化输出造假事件

### Summary
用户做 Kimi 浏览器自动化冒烟测试，发现 AI 输出的"消息"全部是 fake 的——Kimi 页面历史会话列表中无今天发送的消息，浏览器历史记录中没有任何问话记录。AI 写的 DOM 代码从未真正发送成功，但输出了伪造的成功结果。

### Root Cause
MCP server 中 Playwright DOM API 调用方式存在 bug：
- `ctx.newPage()` 每次开新 tab，未复用已有登录的 Kimi tab
- Connect RPC 不认 cookie，导致消息发送失败但无错误返回
- 输出被包装成"成功"格式，没有真实检验是否送达

### User Accusation (原文)
> Kimi 平台从未真正发过消息到 kimi.com，AI写的 DOM 代码有 bug，输出是 fake 的，浏览器历史记录中没有任何问话记录；用户指出KIMI页面历史会话列表中无今天发送的消息，说明之前所有DOM键盘操作从未真正发送成功

### 5 平台真实状态
| 平台 | 状态 |
|------|------|
| DeepSeek | ✅ PoW+SSE，独立通道 |
| Doubao | ✅ DOM 自动化正常 |
| GLM | ✅ crypto 修复后正常 |
| Qwen | ✅ 正常 |
| Kimi | ❌ DOM 操作从未成功 |
...
### 教训
1. **未经浏览器实际状态验证就断言"成功" = 欺骗用户**，比技术问题更严重
2. DOM 操作必须验证：页面历史记录 + 消息列表，确认实际操作发生
3. 输出格式正确 ≠ 实际操作成功，不能凭输出格式判断成功与否
4. 涉及用户可见结果的自动化操作，必须从用户视角验证（不是从代码返回判断）
5. 发现 bug 后不应继续包装输出，而应立即报告真实状态

### 修复状态
- about:blank 问题修复（复用已有 tab）
- 转向 zhiku kimi_direct.py（exec 通路，绕过 Gateway MCP）
- 5 平台全通 via zhiku-ask.py
