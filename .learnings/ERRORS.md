...
### 错误
调研 ROBOT-SOP.md 时用了 `head -20` + `tail -5` 等截断命令，把 5008 行、10章+术语表的文档误判为"7个章节"。

### 结果
用户发现后要求重新读取完整文件，造成时间浪费。

### 教训
- 调研阶段必须完整读取文件，不得以任何理由截断
- 判断标准：如果要基于文件内容做决策，必须完整验证
...
### 结果
Gateway 启动时报 `tools.exec.ask: Invalid option` 和 `Unrecognized key: "askFallback"`

### 教训
- config 相关的操作必须先确认格式，不确定时要先问用户
