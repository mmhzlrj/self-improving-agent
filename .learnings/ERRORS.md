...
## 2026-03-28-A: mdview.py 打开文件后浏览器停在 dashboard.html

### 错误描述
Minimax subagent 调用 mdview.py 打开 markdown 文件，mdview.py 输出"已打开"，但浏览器实际显示 dashboard.html 而非目标文件。

### 症状
- mdview.py 输出：`✅ 已打开，URL: http://127.0.0.1:18999/index.html`
- 浏览器实际：停留在 `dashboard.html` 或无变化

### 根因（三层叠加）

**Layer 1 - webbrowser.open() 默认 new=0**
- `webbrowser.open(url)` 默认 `new=0`，激活已有 tab 而非打开新 URL
...
- mdview.py 和 dashboard_mcp_server.py 共用 18999 端口
- mdview.py 检测到"已有 mdview 进程"就直接调用 webbrowser.open(18999/index.html) 退出
- 但实际 18999 是 dashboard，它对 /index.html 返回 302 重定向到 /dashboard.html
- 修复：mdview 改用独立端口 **18990**

**Layer 3 - 复用逻辑只认进程不认端口**
- `existing_pids` 检查只搜索 `mdview.py` 进程名
- 找到任意 mdview.py 进程就用 PORT 打开，不检查 PORT 是否匹配
- 修复：先用 `socket.connect_ex(('127.0.0.1', PORT))` 确认端口监听

### 修复
1. `PORT = 18999` → `PORT = 18990`
2. `webbrowser.open(url)` → `webbrowser.open(url, new=2)`（两处）
3. 复用检查改为 `socket.connect_ex()` 确认端口监听
...
### Metadata
- Source: error
- Tags: webbrowser, port-conflict, mdview, dashboard

---

## 2026-03-28-B: dashboard 点击 .md 文件打开 2 个标签页

### 错误描述
在 dashboard (http://127.0.0.1:18999/dashboard.html) 里点击任务详情中的 .md 参考文件链接，会打开 2 个标签页。

### 根因
点击 → `fetch('/api/open?file=...')` → dashboard server spawn `mdview.py` → mdview.py 调用 `webbrowser.open()` → **Tab 1** → API 返回 URL → JS 调用 `window.open(url)` → **Tab 2**

mdview.py 自己打开了一个 tab，JS 又打开了一个 tab，双重打开。

### 修复
1. mdview.py 加 `--no-browser` / `-n` 参数：生成 HTML 但不打开浏览器
2. server.py 的 subprocess.Popen 调用 mdview 时加 `--no-browser`
3. dashboard server 重启生效

### 文件
- `tools/mdview.py`（加 --no-browser 参数和条件判断）
- `harness/robot/server.py`（subprocess 加 --no-browser）
- `harness/robot/dashboard_mcp_server.py`（同上）

### Metadata
- Source: error
- Tags: double-tab, subprocess, webbrowser-open, dashboard
...
### 错误
git clone https://github.com/Gen-Verse/OpenClaw-RL.git 超时

### 解决
用 GitHub tarball 下载，或用 gh api。但目前网络限制还未完全解决。

## 2026-03-28: CUDA Toolkit 包名错误

### 错误
`sudo apt install cuda-toolkit-12-9` → 无法定位软件包

### 解决
PyTorch 用 conda 安装，不需要单独装 CUDA Toolkit

## 2026-03-28: sentence-transformers show_progress 参数不兼容

### 错误
ValueError: additional keyword arguments that this model does not use: ['show_progress']

### 解决
去掉 show_progress 参数（5.x 版本已不支持）

## 2026-03-28: subagent heredoc 传文件失败

### 错误
subagent 里用 heredoc 写多行脚本到远程服务器失败

### 解决
先用 write 工具写本地，再用 scp 传远程
