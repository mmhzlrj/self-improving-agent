...
### 错误：mdview.py 端口占用时报错时反复重试，打开 3 个重复标签页

**问题**：`python3 mdview.py` 报错 `OSError: Address already in use`，我连续执行了 3 次，导致浏览器打开 3 个重复标签页（2个 `index.html` + 1个根路径）。

**错误**：
1. 一看到报错就慌张重试，没有先验证页面是否真的打不开
2. 没有先 `curl http://127.0.0.1:18999/` 验证服务是否真的不可用
3. 没有用 `lsof -ti:18999` 查 PID 就想用 `pkill`（会误杀其他进程）
4. 误判了报错内容，以为自己失败了，实际上 mdview 可能已经正常启动

**正确做法**：
1. 报错"端口占用" → 先等 2 秒
2. `curl http://127.0.0.1:18999/` 验证是不是真的不行
3. 确认不行再查 `lsof -ti:18999` 找 PID，只 kill 那一个
4. 耐心理性排查 > 慌张重复尝试

**教训**：
- 遇到报错先验证是不是真的失败了，不能一看到报错就重试
- 多次重复尝试往往比一次耐理性排查更浪费时间

---

### 错误：mdview.py daemon 线程导致僵尸进程

**时间**：2026-03-24 12:20-12:40

**现象**：连续多次执行 `python3 mdview.py` 后，端口 18999 被占用但 `curl` 无法连接，`ps` 显示有多个 mdview.py 进程在运行但实际不提供服务。

**根因**：
1. `start_server()` 在 daemon 线程中运行，端口绑定失败时 `sys.exit(1)` 只退出该线程
2. 主线程继续执行 `while True: time.sleep(3600)` 循环，进程永远不会退出
3. 僵尸进程持有端口但不响应 HTTP 请求
4. 后续启动尝试全部失败（端口被僵尸占用），又产生更多僵尸

**错误链**：
```
端口被占用 → daemon 线程 bind 失败 → sys.exit(1) 只退线程 → 主线程 sleep → 僵尸进程
→ 新启动也失败 → 又是僵尸 → 循环恶化
```

**修复（三层保护）**：
1. **第1层**：启动前 `ps aux | grep mdview.py` 检查已有进程，有则复用不启动新的
2. **第2层**：`threading.Event` 让服务器线程通知主线程启动结果（成功/失败）
3. **第3层**：`urllib.request.urlopen` 验证服务器真的在响应，不响应则 `os._exit(1)` 强杀整个进程

**关键区别**：
- `sys.exit(1)` → 只退出当前线程（daemon 线程中无效）
- `os._exit(1)` → 强制退出整个进程（不留僵尸）

**同时修复**：
- `SO_REUSEADDR` 防止 TIME_WAIT 导致端口冲突
- 第二次启动时直接复用已有服务器，不打开新标签页

**教训**：
- daemon 线程中的异常不会被主线程感知，不能用 `sys.exit` 清理
- 必须用线程间通信（Event/Queue）让主线程知道子线程的状态
- 端口被占用 ≠ 服务器在运行，可能是僵尸进程

---

### 错误：编辑文档时反复尝试失败方案

**时间**：2026-03-24 12:35-12:41

**现象**：修改 mdview.py 防止重复标签页时，反复尝试 5-6 种方案都失败，浪费了大量时间和 token。

**错误过程**：
1. 第一次方案：`is_port_in_use` + `is_server_alive` → urllib 超时不可靠
2. 第二次方案：`SO_REUSEADDR` 在检测函数中 → 检测逻辑反了
3. 第三次方案：杀旧进程重试 → 杀掉了正在工作的服务器
4. 第四次方案：复杂的多层判断 → `os._exit` 还没加
5. 最终方案：`ps aux` 检测已有进程 → 简单有效

**根因**：
- 过度设计：试图处理所有边缘情况（TIME_WAIT、僵尸、端口冲突）
- 没有先想清楚最简单的方案再动手
- 最终有效的方案其实非常简单：检查有没有 mdview 进程，有就复用

**教训**：
- 先想最简单的方案，验证不行再复杂化
- 不要一遇到问题就换方案，先分析根因
- `ps aux | grep` 比复杂的端口检测可靠得多

## [ERR-20260324-001] subagent parallel too many

**Logged**: 2026-03-24T16:45:00+08:00
**Priority**: critical
**Status**: resolved
**Area**: infra

### Summary
一次性并行启动 5 个 subagent，触发了 API 限速，所有 agent 全部失败。

### Error
```
{"error": "missing_brave_api_key", "message": "web_search (brave) needs a Brave Search API key..."}
```
（5 个 subagent 都报同样错误）

### Context
- 操作：sessions_spawn 5 个 subagent，每个 agent 内调用 web_search
- 原因：web_search 没有配置 BRAVE_API_KEY，且并行量太大
- 影响：调研失败，需要重新用 minimax/tavily 分批查询

### Suggested Fix
1. 调研任务不要一次开 5 个 subagent，分批进行
2. 使用 tavily/minimax 替代 web_search
3. 每批调研最多 2 个 subagent

### Metadata
- Reproducible: partial（单独调用 web_search 会成功，并行太多会触发限速）
- Related Files: ROBOT-SOP.md

---

## [ERR-20260325-001] mdview review 解析器三大 bug 导致条目显示异常

**Date**: 2026-03-25
**Severity**: critical
**Area**: tools/mdview.py

### 错误 1: Unicode 正则在 raw string 中不工作
- **文件**：tools/mdview.py 第 438-443 行
- **错误**：正则用了 `r'^\*\*\u539f\u6587\*\*[：:]\s*'` raw string，`\uXXXX` 不被解释
- **后果**："问题/修正方案/来源" 全部不显示

### 错误 2: ## 标题被错误包裹进 item div
- **文件**：tools/mdview.py 第 434-448 行
- **错误**：item content 分割时没有在 `##` 处截断
- **后果**：`## 🟡 中优先级修正` 跑到 item-2 的 div 里

### 错误 3: 解析器只识别 ### 标题，忽略列表项
- **文件**：round1-review.md 原格式
- **错误**：文件用列表 `- **3**.**` 格式
- **后果**：条目 3-12 全部被忽略

### 修复方式
1. 正则改用直接中文：`r'^\*\*原文\*\*[：:]\s*'`
2. 分割 lines 时遇到 `^##\s` 就停止
3. round1-review.md 格式从列表升级为 `### N.` 标题

## mdview: markdown.tables 三级缩进目录渲染丢失（2026-03-25）

### 错误
`| | | 标题内容 |` 格式的表格行在 mdview 中不显示内容。

### 原因
Python `markdown` 库的 `tables` 扩展按管道符 `|` 分割单元格。`| | | content |` 被解析为 4 个单元格（空、空、content、空），但表头只声明了 2 列，多出的列被截断。

### 修复
在 `generate_normal_html()` 中增加 `preprocess_toc()` 预处理，检测 `| 章节 | 标题 |` 格式的目录表格，整体转为 HTML `<table>`，用前导空单元格数判断层级，CSS padding 缩进。详见 `tools/mdview.py`。
