...
用户问题
    │
    ├── alsoAllow ──→  deepseek_deepseek_chat ──→  DeepSeek
    ├── alsoAllow ──→  kimi_kimi_chat ──→  Kimi
    ├── alsoAllow ──→  doubao_doubao_chat ──→  Doubao
    ├── alsoAllow ──→  glm_glm_chat ──→  GLM
    └── alsoAllow ──→  qwen_qwen_chat ──→  Qwen
    │
    ▼
...
| 平台 | 工具（alsoAllow 名） | 模式 | 特点 |
|------|------|------|------|
| DeepSeek | `deepseek_deepseek_chat` | 深度思考+智能搜索 | 推理能力强，联网搜索 |
| Kimi | `kimi_kimi_chat` | K2.5思考模式 | 超长上下文，200万字 |
| Doubao | `doubao_doubao_chat` | 专家模式 | 深度思考，研究级 |
| GLM | `glm_glm_chat` | 思考+联网 | 中文语境强，联网搜索 |
| Qwen | `qwen_qwen_chat` | qwen3.5-plus | 阿里大模型，逻辑清晰 |

**⚠️ 历史记录**：旧版工具名 `webauth_*` 已废除（2026-03-31），统一使用上述 alsoAllow 工具名。
