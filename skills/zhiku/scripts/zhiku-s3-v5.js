...
async function main() {
    ...
    // 智谱单独处理
    const page = await findOrOpen('https://chatglm.cn/', '智谱');
    if (page) {
        await ask(page, QUESTION);
        await collect(page, '智谱');
    }
    
    console.log("=== subagent3 完成 ===");
    await browser.close();
}
