...
**MCP 工具调用方式（2026-03-31 更新）⚠️ 旧方式已废除**

~~旧方式（废除了）：~~
- ~~通过 `webauth_mcp` server 的 `webauth_*` 工具名调用~~
- ~~通过 `sessions_spawn` subagent 间接调用~~
- ~~通过 zhiku skill 脚本调用~~

**最新方式（2026-03-31 起）：**
1. 工具通过 **openclaw-mcp-adapter** 注册到 Gateway
2. 工具名（无前缀，由 adapter 自动处理）：
   - `doubao_doubao_chat`
   - `deepseek_deepseek_chat`
   - `kimi_kimi_chat`
   - `qwen_qwen_chat`
   - `glm_glm_chat`
3. 调用方式：在 `openclaw.json` 的 `alsoAllow` 列表中注册
4. Gateway 重启后生效，main agent 可直接调用

**openclaw.json alsoAllow 配置（当前生效中）：**
```json
"alsoAllow": [
  "doubao_doubao_chat",
  "deepseek_deepseek_chat",
