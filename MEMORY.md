...
**MCP 工具调用方式（2026-03-31 重大更新）⚠️ 旧描述已废除**

~~旧描述（已废除，仅作历史参考）：~~
- ~~工具通过 alsoAllow 注册后，main agent 可直接调用~~
- ~~调用方式：在 `openclaw.json` 的 `alsoAllow` 列表中注册~~

**更新说明（2026-03-31）**：
经过深入源码分析（auth-profiles、tool-policy 模块），发现 alsoAllow 工作原理与预期不同：
- alsoAllow 工具会触发"unknown entries"警告（因不在 core/plugin 列表）
- 但工具实际通过 stripPluginOnlyAllowlist 剥离后，allow=void → 全部允许
- 实际验证：zhiku-ask 脚本成功调用 MCP 工具（DeepSeek+Doubao 返回实时数据）

**alsoAllow 正确配置方式（2026-03-31 确认）：**
```json
{
  "tools": {
    "profile": "full",        // ← 必须用 full，coding 会导致 alsoAllow 工具被 block
    "alsoAllow": [
      "doubao_doubao_chat",
...
**⚠️ 关键教训：alsoAllow 必须放在 `tools` 级别（global），不能放在 `agents.list[0].tools` 级别！**
- `alsoAllow` 在 `agents.list[0].tools` → 触发 `agents.main.tools.allow allowlist contains unknown entries` 警告，且工具可能不生效
- `alsoAllow` 在 `tools`（global）+ profile=full → 工具正常通行

**openclaw.json alsoAllow 配置（2026-03-31 确认生效）：**
```json
"alsoAllow": [
  "doubao_doubao_chat",
  "deepseek_deepseek_chat",
...
**DeepSeek 独立架构（重要）：**
- DeepSeek 是独立 MCP server，不走 Playwright Chrome 页面
- 使用独立 fetch API + PoW 挑战 + SSE 流式响应
- 其他 4 个平台（Doubao/Kimi/GLM/Qwen）走 Playwright DOM 操控
- DeepSeek `[无响应]` 是服务器端崩溃，非 MCP server bug
